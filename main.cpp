...
static struct argp_option options[] =
        {
                ...
                DECLARE_OPT("pls", 'h', NULL,
                            "perform paretolocalsearch (default: no)"),
                DECLARE_OPT("dnsga2a", '2a', NULL,
                            "perform dnsga2a (default: no)"),
                DECLARE_OPT("dnsga2b", '2b', NULL,
                            "perform dnsga2a (default: no)"),
                DECLARE_OPT("dnsga2prp", 'prp', "PROPORTION",
                            "perform dnsga2a (default: 30)"),
...
#define OPT_IRACE_CPU 0x100  /* non-ASCII prevents short option */
                DECLARE_OPT("irace-cpu", OPT_IRACE_CPU, NULL,
                            "Show CPU time in --irace output. Defaults to elapsed wall time."),

                {0}
        };

static double wall_time_strategy() { return time(NULL); }
static double cpu_time_strategy() { return (double)clock() / CLOCKS_PER_SEC; }

static error_t parse_opt(int key, char *arg, struct argp_state *state)
{
    struct ea_parameters *params = static_cast<ea_parameters *>(state->input);
    switch(key)
    {
        ...
        case 'h': params->do_paretolocalsearch = 1; break;
        case '2a': params->dnsga2a = 1; break;
        case '2b': params->dnsga2b = 1; break;
        case 'prp': params->dnsga2prp = arg ? atol(arg) : 0; break;
        case OPT_IRACE_CPU: params->irace_time_strategy = cpu_time_strategy; break;
...
bool check_parameters(struct ea_parameters* params)
{
    ...
    VALIDATE(retval, params->tuning >= 0,
             "Tuning parameter must be between 0 and %ld", 999999);
...
int main(int argc, char**argv)
{
    ...
    params.quiet = 0;
    params.irace = 0;
    params.irace_time_strategy = wall_time_strategy;
    ...
    if(params.irace)
    {
        double (*time)() = params.irace_time_strategy;
